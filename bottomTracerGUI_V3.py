...
class HDF5Annotator:
    def __init__(self, root):
        ...
        # tk.Label(self.menu_frame, text="Output Directory:").grid(row=2, column=1, padx=10, pady=5)
        # self.output_dir_var = tk.StringVar(value=os.getcwd())
        # self.output_dir_entry = tk.Entry(self.menu_frame, textvariable=self.output_dir_var, width=25, justify="center")
        # self.output_dir_entry.grid(row=2, column=2, padx=10, pady=5, sticky="w")
        # self.browseOutput_Button = tk.Button(self.menu_frame, text="Browse", command=self.choose_output_directory)
        # self.browseOutput_Button.grid(row=2, column=3, padx=10, pady=5, sticky="w")
    ...
    def update_button_states(self):
        if self.depth_option.get() == "Off":
            ...
            self.save_button.config(text="",command=self.apply_traced_line,state="disabled")
